...
class TestTopic(unittest.TestCase):
    ...
    def testInicializacao(self):
        topic = Topic(10, 2)
        self.assertEqual(8, topic.getNumeroAssentosNormais(),
                         "Quantidade errada de assentos comuns!")
        self.assertEqual(2, topic.getNumeroAssentosPrioritarios(),
                         "Quantidade errada de assentos prioritários!")
...
#        self.assertThrows(IllegalArgumentException.class ,() -> {new Topic(5, 10);}, "Não foi retornado um erro do tipo IllegalArgumentException!")
    ...
    def testPassageiroPrioriatioComVagaPrioritaria(self):
        topic = Topic(2, 1)
        self.assertTrue(topic.subir(Passageiro("Marlus", 120)),
                        "A topic tem vaga mas o passageiro não foi inserido!")
        self.assertTrue(topic.subir(Passageiro("Eduarda", 19)),
                        "A topic tem vaga mas o passageiro não foi inserido!")
    ...
    def testPassageiroSemPrioridadeComVagaComum(self):
        ...
        eduarda = topic.getPassageiroAssentoNormal(0)
        self.assertIsNotNone(eduarda, "Falha na inserção!")
        self.assertEqual("Eduarda", eduarda.getNome(),
                         "Passageiro sem prioridade nao encontrado")

    def testPassageiroPrioriatioSemVagaPrioritaria(self):
        topic = Topic(2, 1)
        self.assertTrue(topic.subir(Passageiro("Eduarda", 190)),
                        "A topic tem vaga mas o passageiro não foi inserido!")
        self.assertTrue(topic.subir(Passageiro("Guthyerri", 120)),
                        "Tinha uma vaga comum pro idoso ocupar!")
        guthyerri = topic.getPassageiroAssentoNormal(0)
        self.assertIsNotNone(guthyerri, "Passageiro nao encontrado")
        self.assertEqual("Guthyerri", guthyerri.getNome(),
                         "Passageiro com prioridade nao encontrado")

    def testPassageiroSemPrioriadadeSemVagaComum(self):
        topic = Topic(2, 1)
        self.assertTrue(topic.subir(Passageiro("Eduarda", 19)),
                        "A topic tem vaga mas o passageiro não foi inserido!")
        self.assertTrue(topic.subir(Passageiro("Guthyerri", 19)),
                        "Tinha assento prioritário vago pra ele sentar!")
    ...
    def testDescidaTopicVazia(self):
        topic = Topic(2, 1)
        self.assertFalse(topic.descer("Joaquim"),
                         "Removeu alguém sendo que a topic está vazia!")
    ...
    def testDescidaDePassageiro(self):
        ...
        self.assertEqual(2, topic.getVagas(
        ), "Removeu uma posição do array, deveria somente setar como null!!!")

    def testDescerPassageiroNaoSubiu(self):
        topic = Topic(2, 1)
        topic.subir(Passageiro("Guthyerri", 19))
        topic.descer("Guthyerri")
        self.assertFalse(topic.descer("Zé Adolfo"),
                         "Removeu alguém que não estava na topic!")

    def testCapacidade(self):
        topic = Topic(2, 1)
        topic.subir(Passageiro("Marlus", 12))
        topic.subir(Passageiro("Guthyerri", 130))
        self.assertFalse(topic.subir(Passageiro("Hermilson", 16)),
                         "Tamanho da Topic foi estourado!")

    def testMostrarVagas(self):
        topic = Topic(5, 3)
        self.assertEqual(5, topic.getVagas(),
                         "Quantidade de vagas disponíveis está errada!")
        topic.subir(Passageiro("Bode", 5))
        self.assertEqual(4, topic.getVagas(),
                         "Quantidade de vagas disponíveis está errada!")

    def testMostrarTopicVazia(self):
        topic = Topic(10, 5)
        self.assertEqual("[@ @ @ @ @ = = = = = ]",
                         topic.toString(), "Sua lista está errada!")
